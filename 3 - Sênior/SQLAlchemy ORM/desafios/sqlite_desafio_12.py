...
class Jogador(Base):
    ...
    def __repr__(self):
        return "%s %s - %s" % (self.nome, self.placar, self.time)
...
try:
    ...
    for jogador in sorted(jogadores, key= lambda obj: (-obj.placar, obj.nome)):
        print(jogador)
except:
    session.rollback()
    raise
...
try:
    time_nome = input()
    time = session.query(Time).filter(Time.nome==time_nome).first()
